...
const GOOGLE_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycby3O24IGk7-5fj5RhqWK86XOsW9Q_7CpiO1Iob4Ehq8GglVgm1V5LzxT-_RhGjAVCADGg/exec";
const OFFICIAL_APPLICATION_URL =
  "https://drive.google.com/file/d/1AIQKcH8EiHmJTBOxVPY2FMPEK8h9w800/view?usp=sharing";
const RHS_CONTACT_EMAIL = "myresidentialhousingsolutions@gmail.com";
const RHS_CONTACT_PHONE = "316-393-5720";
const RHS_CONTACT_SUBJECT = "HOME 80 Fast Track Inquiry";
...
function buildBuyerSummaryText(form) {
  return [
    ...
    "READINESS:",
    ...readinessItems.map(
      (item) => `- ${item}: ${form.readiness[item] || "not answered"}`
    ),
    "",
    "DOCUMENTS:",
    ...requiredDocs.map(
      (doc) => `- ${doc}: ${form.documents[doc] ? "checked" : "not checked"}`
    ),
    "",
    "PROCESS TRACKER:",
    ...progressSteps.map(
      (step) => `- ${step}: ${form.progress[step] ? "checked" : "not checked"}`
    ),
    "",
    "LENDER:",
...
function StatusBadge({ tone = "neutral", children }) {
  ...
  return (
    <div
      className={`rounded-full px-4 py-2 text-sm font-medium ring-1 ${toneClasses[tone]}`}
    >
      {children}
    </div>
  );
...
export default function RHSHOME80FastTrackWizard() {
  ...
  const readinessCount = Object.values(form.readiness).filter(
    (v) => v === "yes"
  ).length;
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (!form.householdSize.trim())
        nextErrors.householdSize = "Enter household size.";
    }

    if (step === 2) {
      if (Object.keys(form.readiness).length < 4)
        nextErrors.readiness = "Answer at least 4 readiness questions.";
    }

    if (step === 3) {
      if (docsCount < 3)
        nextErrors.documents = "Check at least 3 document items to continue.";
    }

    if (step === 4) {
      if (
        form.lenderEmail &&
        !/^\S+@\S+\.\S+$/.test(form.lenderEmail)
      )
        nextErrors.lenderEmail = "Enter a valid lender email.";
    }
  ...
  const toggleProgress = (step) => {
    setForm((prev) => ({
      ...prev,
      progress: { ...prev.progress, [step]: !prev.progress[step] },
    }));
  };

  const exportSummary = () => {
    const body = buildBuyerSummaryText(form);
    openMailto(
      RHS_CONTACT_EMAIL,
      `HOME 80 Fast Track Summary - ${form.fullName || "Buyer"}`,
      body
    );
    setSubmitNotice("Opened your email app with the buyer summary ready to send.");
  };
  ...
  const contactRHS = () => {
    const body = [
      ...
      "",
      `Phone: ${RHS_CONTACT_PHONE}`,
    ].join("\n");

    openMailto(RHS_CONTACT_EMAIL, RHS_CONTACT_SUBJECT, body);
  };

  const continueToOfficialApplication = () => {
    window.open(OFFICIAL_APPLICATION_URL, "_blank", "noopener,noreferrer");
  };
  ...
  const reviewWarnings = [
    ...
    readinessCount < 4 &&
      "Buyer may still need prep before opening the official application.",
  ].filter(Boolean);
                ...
                <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="max-w-2xl">
                      ...
                      <h1 className="mt-3 text-3xl font-bold tracking-tight md:text-5xl">
                        HOME 80 Fast Track Wizard
                      </h1>
                      <p className="mt-3 max-w-2xl text-sm leading-6 text-white/90 md:text-base">
                        A submission-ready buyer workflow that saves progress,
                        validates required fields, and creates a clean handoff
                        into the official HOMEownership 80 application.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 rounded-[24px] bg-white/10 p-5 backdrop-blur md:min-w-[280px]">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={manualSave}
                        className="inline-flex items-center justify-center gap-2 rounded-[22px] bg-white/10 px-4 py-3 text-sm font-semibold text-white transition hover:bg-white/15"
                      >
                ...
                {stepsMeta.map((step, idx) => (
                  ...
                  >
                    <StepPill
                      ...
                      complete={idx < currentStep || (submitted && idx === 5)}
                      title={step.title}
                      icon={step.icon}
                      index={idx}
                    />
  ...
  <div>
    ...
    <p className="mt-1 text-sm text-stone-700">
      This wizard is your shortcut. It is not a replacement for the official HOME 80 packet.
    </p>
  </div>
    ...
    Start here
                  ...
                  <div className="mt-6 grid gap-4 md:grid-cols-3">
                    {[
                      [
                        "1",
                        "Complete this guide",
                        "Answer a few quick questions and track your progress.",
                      ],
                      [
                        "2",
                        "Gather documents",
                        "Use the checklist to get everything ready before you start the official packet.",
                      ],
                      [
                        "3",
                        "Submit ready",
                        "Create a clean handoff summary for RHS and move into the official application.",
                      ],
                    ].map(([n, title, text]) => (
                      <div
                        key={title}
                ...
                >
                  <div className="space-y-3">
                    {readinessItems.map((item) => (
                      <div
                        key={item}
                        className="flex flex-col gap-3 rounded-[22px] border border-stone-200 px-4 py-4 transition hover:bg-stone-50 md:flex-row md:items-center md:justify-between"
                      >
                        ...
                        <div className="flex gap-3">
                          <label className="inline-flex items-center gap-2 rounded-full border border-stone-300 bg-white px-4 py-2 text-sm hover:border-emerald-700 hover:text-emerald-800">
                            <input
                              type="radio"
                              name={item}
                              checked={form.readiness[item] === "yes"}
                              onChange={() =>
                                setForm((prev) => ({
                                  ...prev,
                                  readiness: {
                          ...
                          <label className="inline-flex items-center gap-2 rounded-full border border-stone-300 bg-white px-4 py-2 text-sm hover:border-rose-700 hover:text-rose-800">
                            <input
                              type="radio"
                              name={item}
                              checked={form.readiness[item] === "no"}
                              onChange={() =>
                                setForm((prev) => ({
                                  ...prev,
                                  readiness: {
                ...
                >
                  <div className="grid gap-4 md:grid-cols-2">
                    {requiredDocs.map((doc) => (
                      <label
                        key={doc}
                        className="flex items-center gap-3 rounded-[22px] border border-stone-200 bg-stone-50 px-4 py-4 transition hover:bg-white hover:shadow-sm"
                      >
                        <input
                          type="checkbox"
                          checked={Boolean(form.documents[doc])}
                ...
                >
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <Field
                      ...
                      onChange={(e) =>
                        setField("loanOfficer", e.target.value)
                      }
                      icon={User}
                    />
                    <Field
                      ...
                      onChange={(e) =>
                        setField("lenderPhone", e.target.value)
                      }
                      icon={Phone}
                    />
                    <div className="md:col-span-2">
                      <Field
                        ...
                        onChange={(e) =>
                          setField("lenderEmail", e.target.value)
                        }
                        icon={Mail}
                  ...
                  <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
                        ...
                        <div className="mt-3 space-y-2 text-sm leading-6 text-stone-700">
                          <p>
                            ...
                            <span className="font-semibold">
                              {readinessCount} of {readinessItems.length}
                            </span>
                          </p>
                          <p>
                            Documents checked:{" "}
                            <span className="font-semibold">
                              {docsCount} of {requiredDocs.length}
                            </span>
                          </p>
                          <p>
                            Buyer process steps checked:{" "}
                            <span className="font-semibold">
                              {progressCount} of {progressSteps.length}
                            </span>
                          </p>
                        </div>
                        {reviewWarnings.length ? (
                          <div className="mt-4 rounded-[20px] bg-amber-50 p-4 text-sm text-amber-900 ring-1 ring-amber-200">
                            <div className="mb-2 flex items-center gap-2 font-semibold">
                              <AlertCircle className="h-4 w-4" /> Review before
                              sending
                            </div>
                    ...
                    <div className="space-y-4 rounded-[24px] bg-stone-900 p-6 text-white shadow-xl">
                      ...
                      <p className="text-sm leading-6 text-white/85">
                        Use the buttons below to submit to RHS, email the buyer
                        summary, save progress, or move into the official
                        application step.
                      </p>
                      <div className="grid gap-3">
                        <button
                          onClick={handleSubmit}
                          className="inline-flex items-center justify-center gap-2 rounded-[22px] bg-emerald-700 px-5 py-3.5 text-base font-semibold text-white shadow-lg transition hover:translate-y-[-1px] hover:opacity-95"
                        >
                          <CheckCircle2 className="h-4 w-4" /> Submit to RHS
                        </button>

                        <button
                          onClick={exportSummary}
                          className="inline-flex items-center justify-center gap-2 rounded-[22px] bg-white/10 px-5 py-3.5 text-base font-semibold text-white ring-1 ring-white/10 transition hover:bg-white/15"
                        >
                          <Mail className="h-4 w-4" /> Email Buyer Summary
                        </button>
                        ...
                        <button
                          onClick={continueToOfficialApplication}
                      ...
                      {submitNotice ? (
                        <div className="rounded-[20px] bg-white/10 p-4 text-sm text-white/90 ring-1 ring-white/10">
                          {submitNotice}
                        </div>
                      ) : null}
                    </div>
            ...
            <div className="flex items-center justify-between gap-4 rounded-[28px] bg-white px-5 py-4 shadow-xl ring-1 ring-stone-200 md:px-6">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-2xl border border-stone-300 px-4 py-3 font-medium text-stone-800 transition hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
                <ChevronLeft className="h-4 w-4" /> Back
              </button>

              <div className="text-center text-sm text-stone-600">
                Step <span className="font-semibold text-stone-900">{currentStep + 1}</span> of{" "}
            ...
            <div className="sticky top-6 space-y-6">
              <section className="overflow-hidden rounded-[28px] bg-white shadow-xl ring-1 ring-stone-200">
                <div className="border-b border-stone-200 px-6 py-5">
                  ...
                  <p className="mt-1 text-sm text-stone-600">
                    A simple, motivating snapshot for buyers.
                  </p>
                </div>
              ...
              <section className="overflow-hidden rounded-[28px] bg-white shadow-xl ring-1 ring-stone-200">
                <div className="border-b border-stone-200 px-6 py-5">
                  ...
                  <p className="mt-1 text-sm text-stone-600">
                    Keeps the next steps visible and motivating.
                  </p>
                </div>
                <div className="space-y-3 p-6">
                  {progressSteps.map((step, idx) => (
                    <label
                      key={step}
                      className="flex items-center gap-3 rounded-[22px] border border-stone-200 px-4 py-4 transition hover:bg-stone-50"
                    >
                      <input
                        type="checkbox"
                        checked={Boolean(form.progress[step])}
                        onChange={() => toggleProgress(step)}
                        className="h-5 w-5 rounded"
                      />
                      <div>
                        <div className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-500">
              ...
              <section className="overflow-hidden rounded-[28px] bg-stone-900 text-white shadow-xl">
                <div className="p-6">
                  ...
                  <div className="mt-4 space-y-3 text-sm leading-6 text-white/85">
                    ...
                    <p>• Sends submissions to RHS and stores them in Google Sheets.</p>
                  </div>
              ...
              <section className="overflow-hidden rounded-[28px] bg-white shadow-xl ring-1 ring-stone-200">
                <div className="p-6">
                  ...
                  <div className="mt-4 space-y-2 text-sm leading-6 text-stone-700">
                    ...
                    <p>Email: {RHS_CONTACT_EMAIL}</p>
                    <p>Phone: {RHS_CONTACT_PHONE}</p>
                    <p>
                      Use this guided tool as your shortcut, then complete the
                      official HOME 80 application packet.
                    </p>
                  </div>
                  <button
                    onClick={contactRHS}
